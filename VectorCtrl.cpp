...
//コンストラクタ
VectorCtrl::VectorCtrl(DigitalOut *digi, PwmOut *pwm): _digi(digi), _pwm(pwm) {
	//initialize
	l_limit = MOTOR_LOWER_LIMIT;
	for(int i=0; i<4; ++i)
		for(int j=0; j<2; ++j)
			out[i][j] = 0.0;
	prop = PWMOUT_PROPORTION;
}
...
void VectorCtrl::move(double x, double y, double theta, double omega) {
	...
	for(int i=0; i<4; ++i) {
		//台形制御
		if( (result[i] < 0.0 ? 1 : 0) != _digi[i] && (out[i][0] > 0.0)) {
			//動かしたい方向と逆に回っている場合は減速
			out[i][0] -= prop;
			out[i][1] = 1.0 - out[i][0];
		} else {
			_digi[i] = (result[i] < 0.0 ? 1 : 0); //モーターの回転方向を設定

			if( out[i][0] < fabs(result[i]) && out[i][0] < h_limit ) {
				//目標値か上限より遅ければ加速
				out[i][0] += prop;
				out[i][1] = 1.0 - out[i][0];
			} else {
		...
		_pwm[i].write( out[i][_digi[i]] ); //モーターのPWM出力
...
void VectorCtrl::proportion(double value) {
	if(value < 0.0 || value > 1.0) {
		prop = PWMOUT_PROPORTION; //範囲外の場合は初期値に設定
	} else {
		prop = value; //割合の値を設定
	}
}

//emergency stop //足回り緊急停止
void VectorCtrl::emergency() {
	for(int i=0; i<4; ++i) {
		_digi[i] = 0;
		_pwm[i].write(0.0);
	}
...
//circuit debug output //回路デバッグ用にモーターを動かすだけ
void VectorCtrl::debug(bool n, double power) {
	if(!n) { //motor rotate
		for(int i=0; i<4; ++i) {
			_digi[i] = 0;
			_pwm[i].write(power);
		}
	} else { //motor rotate invert
		for(int i=0; i<4; ++i) {
			_digi[i] = 1;
			_pwm[i].write(1 - power);
		}
	}
}
