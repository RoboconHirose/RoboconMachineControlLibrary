...
void ctrl::VectorCalculator::calculateVector(ctrl::Vector &vectorWheel, ctrl::VectorMove &vectorMove,
                                             ctrl::WheelAttr &wheelAttr) {
	// ベクトルの強さに応じて動きを変更
	if (vectorMove.getMagnitude() > EPSILON) {
		// 回転操作なし
		if (std::fabs(vectorMove.getSteer()) < EPSILON) {
			// 直進
			vectorWheel.setMagnitude(vectorMove.getMagnitude());
			vectorWheel.setAngle(vectorMove.getAngle());
			...
			if (std::fabs(vectorMove.getSteer()) > 0.95) { // フルステアでは計算がエラーになるので、例外処理
				steerR = 0;
			} else {
				steerR = wheelAttr.getWheelDist() / std::tan(vectorMove.getSteer() * M_PI_2);
			}
			// 旋回中心が位置する直線の角度
			cAngle = vectorMove.getAngle() + M_PI_2;
			// 旋回中心の座標を求める
			cx = steerR * std::cos(cAngle) + offset;
			cy = steerR * std::sin(cAngle);
			// 車両原点での旋回半径
			turnR = std::sqrt(cx * cx + cy * cy);
			if (vectorMove.getSteer() < 0) { // steerRと正負を揃える
				turnR = -turnR;
			}
			...
			// turnSpdは基準位置の速度（常に正）
			turnSpeed = std::fabs(turnR) / (std::fabs(turnR) + wheelAttr.getWheelDist()) * vectorMove.getMagnitude();
			...
			if ((std::fabs(vx) < 5) && (std::fabs(vy) < 5)) {
				...
				vectorWheel.setMagnitude(0.0);
				vectorWheel.setAngle(0.0);
				...
				vectorWheel.setAngle(vAngle + M_PI_2);
				// 旋回半径の差による速度の増減

				if (std::fabs(turnR) > 10) { // 車両中心の旋回半径が0でないので緩旋回
					speedFact = std::sqrt(vx * vx + vy * vy) / turnR;
					// ホイール位置での移動速度
					vectorWheel.setMagnitude(turnSpeed * speedFact);

				} else if (vectorMove.getSteer() < 0) { // 旋回半径が極めて小さいので超信地旋回（右旋回）
